...
class HLIPController:
    ...
    def __init__(
        ...
        # self.Kp = 100 * np.eye(5)
        # self.Kd = 2 * np.sqrt(self.Kp) * np.eye(5)
    ...
    def gaitController(self, q:np.ndarray, q_pos_ctrl:np.ndarray, q_vel_ctrl:np.ndarray, t:float, right_cont:bool, left_cont:bool, stf_ang_mom:float) -> tuple:
        ...
        self.logger.write(np.hstack((
            ...
            impact, stf_ang_mom, ang_mom, x_ssp_curr_L, x_ssp_impact_L, x_ssp_impact_ref_L, q_ff_ref_gravcomp, tau, ddq, grf, dde - dded, 0.5*np.inner(dde - dded, dde - dded), M.reshape((-1,)), H, Jh.reshape((-1))
        )))
        ...
        # Jmh = Jminv @ Jh @ Minv
        # Ah = -Jmh @ B
        # bh = Jmh @ H - Jminv @ dJh @ q_vel_ctrl

        # Abar = Minv @ (B + Jh.T @ Ah)
        # bbar = Minv @ (Jh.T @ bh - H)

        # Rgrf = np.array([[0, -1], [1, -self.mu_friction], [-1, -self.mu_friction]])
        # Agrf = Rgrf @ Ah
        # bgrf = -Rgrf @ bh
        ...
        # Pr2 = scipy.sparse.csc_matrix(Ar1.T @ Ar1)
        # Qr2 = Ar1.T @ (br1 - dded)

        # Ar2 = scipy.sparse.csc_matrix(np.vstack((Agrf, np.eye(4))))
        # UBr2 = np.hstack((bgrf, np.ones((4,)) * self.tau_max))
        # LBr2 = np.hstack((-np.inf * np.ones_like(bgrf), np.ones((4,)) * -self.tau_max))

        # tsc_qp2 = osqp.OSQP()
        # tsc_qp2.setup(P=Pr2, q=Qr2, A=Ar2, u=UBr2, l=LBr2, eps_rel=1e-9)

        # res2 = tsc_qp2.solve()

        # tau2 = res2.x
        # grf2 = Ah @ tau2 + bh
        # ddq2 = Minv @ (B @ tau2 + Jh.T @ grf2 - H)

        # dde2 = Ar1 @ tau2 + br1

        # # Check if solution is correct
        # obj2 = 0.5*np.inner(dde2 - dded, dde2 - dded)
        # sol_obj2 = res2.info.obj_val
        # sol_obj_expected2 = 0.5 * tau2.T @ Pr2 @ tau2 + Qr2.T @ tau2
        # obj_expected2 = sol_obj_expected2 + 0.5 * np.inner(dded - br1, dded - br1)