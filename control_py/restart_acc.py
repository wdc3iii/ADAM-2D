...
def main():
    ...
    y0, yd0, yF, ydF = controller.getNominalS2S(u, x_pre_ref, u)
    ...
    for ii in range(N):
        ...
        if ii >= 1:
            ...
            q0, _ = controller.adamKin.solveIK(Q_IK, y, False)
            # Modify for contact
            mjInt.setState(q0, QD_ZERO)
            mjInt.forward()
            ft_pos = mjInt.getFootPos()
            q0[1] -= ft_pos[1][1] - controller.adamKin.getContactOffset(q0, False)
            q0[1] += dz
            print(dz)
            ...
            qd0 = controller.adamKin.solveIKVel(q0, yd, False)
        ...
        stf = controller.cur_stf

        while n < N - ii:
            if mjInt.time() - t_vis > 1 / 60:
                t_vis = mjInt.time()
                mjInt.updateScene()
            t = mjInt.time()
            ...
            if controller.cur_stf == stf:
                stf = not controller.cur_stf
                n += 1
                
                if n == 1:
                    qCpos = controller.adamKin.calcOutputs(qpos, True)
                    qCvel = controller.adamKin.calcDOutputs(qpos, qvel, True)

                    ft_pos = mjInt.getFootPos()
                    dz = ft_pos[0][1] - controller.adamKin.getContactOffset(qpos, True)
